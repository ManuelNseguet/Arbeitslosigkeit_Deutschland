...
def plot_women_vs_men_trend(df):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "01_women_vs_men_trend.png", dpi=DPI)
    plt.close()


def plot_gender_gap_trend(df):
    plt.figure(figsize=FIG_SIZE)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "02_gender_gap_trend.png", dpi=DPI)
    plt.close()


def plot_overall_unemployment_trend(df):
    plt.figure(figsize=FIG_SIZE)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "03_overall_unemployment_trend.png", dpi=DPI)
    plt.close()
